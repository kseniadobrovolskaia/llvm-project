...
#include "ISSnake.h"
#include "ISSnakeMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {ISSnake::R9, ISSnake::R10, ISSnake::R11,
                                    ISSnake::R12};

void ISSnakeTargetLowering::ReplaceNodeResults(
...
ISSnakeTargetLowering::ISSnakeTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(ISSnake::R1);
...
#include "ISSnakeGenCallingConv.inc"
...
SDValue
ISSnakeTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_ISSnake);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, ISSnake::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(ISSnakeISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_ISSnake);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const ISSnakeSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const ISSnakeTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// ISSnake formal arguments implementation
SDValue ISSnakeTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_ISSnake);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &ISSnake::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
...
bool ISSnakeTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_ISSnake))
    return false;
...
SDValue
ISSnakeTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                   bool IsVarArg,
  ...
  const ISSnakeSubtarget &STI = MF.getSubtarget<ISSnakeSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_ISSnake);
  ...
  return DAG.getNode(ISSnakeISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue ISSnakeTargetLowering::PerformDAGCombine(SDNode *N,
...
bool ISSnakeTargetLowering::isLegalAddressingMode(const DataLayout &DL,
